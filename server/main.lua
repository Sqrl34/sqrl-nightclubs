...
RegisterCommand('clearprop', function(source)
    TriggerClientEvent('nightclubs:client:removeipl', source)
end, false)
...
--[[RegisterCommand('setinfo', function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local data = json.encode(ClubData)
    local id = MySQL.insert('INSERT INTO `nightclubs` (citizenid, metadata) VALUES (?, ?)', {
        Player.PlayerData.citizenid, data })
end,false)]]
--
...
RegisterNetEvent('nightclubs:server:handouts', function(ClubData, Employee, percentage)
    ...
    for k,v in pairs(Config.Earnings.upgrades) do
        if Config.Earnings.upgrades[k].bonus then
            for m, v in pairs(Config.Earnings.upgrades[k].types) doe'])
